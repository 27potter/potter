...
admin = ["u5feac8905068ba8694f94a7c274118ff"]
sider = {}
alamat = {
    "linkimage": "",
    "linkvid": ""
...
def helpmessages():
    if settings['setKey'] == True:
        ...
        key = ''
    helpMessages =   "╔══[ Help Message ]" + "\n" + \
                    "╠ " + key + "ChangeVideoProfile" + "\n" + \
                    "╚══[ {} ]" + "\n" + \
                    "Type 「getlinkvideo」 to get Link Video" + "\n" + \
                    "Type 「getlinkpict」 to get Link Picture"
    return helpMessages
def helpmessagess():
    if settings['setKey'] == True:
        key = settings['keyCommand']
        ...
        key = ''
    helpMessagess =   "╔══[ Get Link Pict ]" + "\n" + \
                    "╠ " + key + " You can upload your pict in website\ntinypic.com " + "\n" + \
                    "╚══[ {} ]"
    return helpMessagess
def helpmessagesss():
    if settings['setKey'] == True:
        key = settings['keyCommand']
    else:
        key = ''
    helpMessagesss =   "╔══[ Get Link Video YouTube ]" + "\n" + \
                    "╠ " + key + "1. Copy Link Video YouTube" + "\n" + \
                    "╠ " + key + "2. Paste Link in website saveitoffline.com" + "\n" + \
                    "╠ " + key + "3. Copy Link Download Video" + "\n" + \
                    "╚══[ {} ]"
    return helpMessagesss
                    ...
                    if msg.contentType == 0:
                        if text is None:
                            ...
                            if cmd == "helps":
                                ...
                                client.sendMessage(to, str(helpMessages))
                            elif cmd == "getlinkpict":
                                helpMessagess = helpmessagess()
                                client.sendMessage(to, str(helpMessagess))
                            elif cmd == "getlinkvideo":
                                helpMessagesss = helpmessagesss()
                                client.sendMessage(to, str(helpMessagesss))
                            elif cmd == "off bot":
                                client.sendMessage(to, "Success Shutdown Bot")
                                sys.exit("[ INFO ] BOT SHUTDOWN")
                            elif cmd.startswith ('cvp'):
                              url = alamat["linkimage"]
                              url2 = alamat["linkvid"]
                              client.sendMessage(to, "sedang proses....")
                              video = client.downloadFileURL(url2, saveAs="video.mp4")
                              client.sendMessage(to, "sukses download video......\ntunggu beberapa saat..")
                              image = client.downloadFileURL(url, saveAs="image.jpg")
                              ...
                              if spl in [""," ","\n",None]:
                                client.sendMessage(msg.to, "Gagal set link video")
                              else:
                                alamat["linkvid"] = spl
                                client.sendMessage(msg.to, "link video telah diset :\n\n{}".format(str(spl)))
                            elif 'linkimage: ' in msg.text:
                              spl = msg.text.replace('linkimage: ','')
                              if spl in [""," ","\n",None]:
                                client.sendMessage(msg.to, "Gagal set link foto")
                              else:
                                alamat["linkimage"] = spl
                                client.sendMessage(msg.to, "link foto telah diset :\n\n{}".format(str(spl)))    
                            elif cmd == "tts":
                                helpTextToSpeech = helptexttospeech()
                                client.sendMessage(to, str(helpTextToSpeech))
                            elif cmd =="memelist":
                                helpMemelist = helpmemelist()
                                client.sendMessage(to, str(helpMemelist))
                            elif cmd == "translate":
                              ...
                              client.sendVideoWithURL(to, data["first_video"])
                                ...
                                client.sendMessage(to, "Bot Already Running {}".format(str(runtime)))
                            elif cmd == "restart":
                              client.sendMessage(to, "Restarting...")
                              client.sendMessage(to, "Success Restart Bot")
                              restartBot()
                            elif cmd == "cek error":
                              with open('errorLog.txt', 'r') as er:
                                error = er.read()
                                client.sendFooter(to, str(error))
# Pembatas Script #
                            elif cmd == "memberlist":
                              if msg.toType == 2:
...
#----------------------#
                              ...
                              nub = os.popen('ls')
                              programmer = nub.read()
                              client.sendMessage(to, "{}".format(programmer))
                              programmer.close()
                            elif cmd == "ciduk on":
                              sider[to] = []
                              client.sendMessage(to, "cek sider diaktifkan")
                            elif cmd == "ciduk off":
                              if to in sider:
                                del sider[to]
                                client.sendMessage(to, "cek sider dinonaktifkan")
                              else:
                                client.sendMessage(to, "cek sider belum diaktifkan")
                            elif cmd == "ciduk reset":
                              if to in sider:
                                sider[to] = []
                                client.sendMessage(to, "cek sider telah di reset")
                              else:
                                client.sendMessage(to, "cek sider belum diaktifkan")
                                ...
                                client.sendFooter(to, str(result, icon, name, link))
                                    ...
                                    for target in targets:
                                        try:
                                            ...
                                            for ls in lists:
                                                ...
                                                client.sendMessage(to, "「Success Kick」\n{}".format(str(contact.displayName)))
                                        except Exception as e:
                                ...
                                try:
                                    ...
                                    data = r.text
                                    data = json.loads(data)
                                    client.sendImageWithURL(msg.to, str(data["image"]))
                                except Exception as error:
                                    logError(error)
                              ...
                              client.sendMessage(to, str(ret_))
                                ...
                                if client != None:
                                 ...
                                 client.sendContact(to, "ue10de0a302f521d008e7b4efa9abd842,'")
# Pembatas Script
                                ...
                                path = "http://dl.profile.line-cdn.net/" + group.pictureStatus
                                client.sendImageWithURL(to, path)
                                client.sendMessage(to, "welcome to {}".format(str(group.name)))
...
# Pembatas Script #
                                ...
                                for i in range(int(sep[1])):
                                  client.sendMessage(to, str(text.replace(sep[0] + " " + sep[1] + " ", ""))) 
                            elif cmd == "changepicture":
                               settings["changePictureProfile"] = True
                               client.sendMessage(to, "Insert The Picture")
                              ...
                              for a in range(k+1):
                                ...
                                for target in targets:
                                    try:
                                        ...
                                        client.sendMessage(msg.to,"Failed To Add Target")
                                        break
                                ...
                                for target in targets:
                                    try:
                                        ...
                                        client.sendMessage(msg.to,"Success Delete Target")
                                        break
                                    except:
                                        client.sendMessage(msg.to,"Failed Delete Target")
                                        break

                            elif cmd == "mimiclist":
                                if settings["mimic"]["target"] == {}:
                                    client.sendMessage(msg.to,"Nothing Target")
                                else:
                                    mc = "╔══[ Mimic List ]"
...
# Pembatas Script #
                                ...
                                try:
                                    ...
                                    if data[0] != "" and data[1] != "" and data[2] != "":
                                        try:
                                            client.sendLocation(to, data[1], data[2], data[0], location)
                                        except Exception as error:
                                ...
                                try:
                                    ...
                                    if data["find"] == True:
                                        if data["media"]["mediatype"] == 1:
                                            client.sendImageWithURL(msg.to, str(data["pict_url"]["1"]))
                                        if data["media"]["mediatype"] == 2:
                                            client.sendVideoWithURL(msg.to, str(data["video_url"]["1"]))
                                        ret_ = "╔══[ Info Post ]"
...
# Pembatas Script #
                        ...
                        if settings["changePictureProfile"] == True:
                            ...
                            client.sendMessage(to, "Success change pict profile")
                        if msg.toType == 2:
                            if to in settings["changeGroupPicture"]:
                                path = client.downloadObjectMsg(msg_id)
                                settings["changeGroupPicture"].remove(to)
                                client.updateGroupPicture(to, path)
                                client.sendMessage(to, "Success change pict group :)")
                            ...
                            for mention in mentionees:
                                if clientMid in mention["M"]:
                                    if settings["autoRespon"] == True:
                                        sendMention(sender, " Saya hanyalah client @!", [sender])
                                    break
            except Exception as error:
                logError(error)
                traceback.print_tb(error.__traceback__)
